...
#define X_APPLY_DIR(v, Q)                    \
  ...
  {                                          \
    X_DIR_WRITE(v);                          \
    X2_DIR_WRITE((v) != INVERT_X2_VS_X_DIR); \
  } while (0)
...
#define Z_APPLY_STEP(v, Q)                                                                                 \
  ...
  {                                                                                                        \
    if (Z_HOME_DIR > 0)                                                                                    \
    {                                                                                                      \
      if (!(TEST(endstops.old_endstop_bits, Z_MAX) && (count_direction[Z_AXIS] > 0)) && !locked_z_motor)   \
        Z_STEP_WRITE(v);                                                                                   \
      if (!(TEST(endstops.old_endstop_bits, Z2_MAX) && (count_direction[Z_AXIS] > 0)) && !locked_z2_motor) \
        Z2_STEP_WRITE(v);                                                                                  \
    }                                                                                                      \
    else                                                                                                   \
    {                                                                                                      \
      if (!(TEST(endstops.old_endstop_bits, Z_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z_motor)   \
        Z_STEP_WRITE(v);                                                                                   \
      if (!(TEST(endstops.old_endstop_bits, Z2_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z2_motor) \
        Z2_STEP_WRITE(v);                                                                                  \
    }                                                                                                      \
  }                                                                                                        \
  else                                                                                                     \
