...
/**
     ...
     */
#define MENU_MULTIPLIER_ITEM(type, label, args...) \
  do                                               \
  {                                                \
    _MENU_ITEM_PART_1(type, label, ##args);        \
...
{
  if (temph > 0)
    thermalManager.setTargetHotend(temph, endnum);
#if TEMP_SENSOR_BED != 0
...
{
  _lcd_preheat(0, preheatHotendTemp1, preheatBedTemp1, preheatFanSpeed1);
}
...
{
  _lcd_preheat(0, 0, preheatBedTemp1, preheatFanSpeed1);
}
...
static void menu_action_submenu(screenFunc_t func)
{
  lcd_save_previous_menu();
...
int lcd_strlen_P(const char *s)
{
  ...
  while (pgm_read_byte(s))
  {
    ...
    j++;
#else
    if ((pgm_read_byte(s) & 0xC0u) != 0x80u)
      j++;
#endif
    s++;
  }
...
void lcd_update()
{
  ...
  if (ELAPSED(ms, next_lcd_update_ms))
  {
    ...
    if (currentScreen == lcd_status_screen && !lcd_status_update_delay--)
    {
